...
gui.h=uicontrol('Style', 'pushbutton', 'String','Record','Position',[gui.posbu(1)+130 135 60 30], 'Callback','obj = (audiorecorder(Fs,8,1));recordblocking(obj, 2);gui.x=getaudiodata(obj);gui.Amp=max(abs(gui.x));gui.x=gui.x./gui.Amp;figure(2);subplot(211);plot(gui.x);title(gui.tit1);y=[gui.x;zeros(round(gui.d*Fs),1)]+gui.a.*[zeros(round(gui.d*Fs),1);gui.x];gui.Amp=max(abs(y));y=y./gui.Amp;y=rot90(y);xlim([0 length(y)]);subplot(212);plot(y);title(gui.tit2);xlim([0 length(y)]);');
